...
let userNine = todo.filter(item => item.userId === 9);
console.log(userNine);


//find the number of user 9's incomplete tasks
let userNineIncompleteTasks = todo.filter(item => {
    if (item.userId === 9 && item.completed === false) {
        return true;
    } else {
        return false;
    }});

let incompleteTasks = userNineIncompleteTasks.length;
